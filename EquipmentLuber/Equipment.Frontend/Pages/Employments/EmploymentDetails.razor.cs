...
namespace Equipment.Frontend.Pages.Employments
{
    public partial class EmploymentDetails
    {
        ...
        private async Task DeleteAsync(Employee employee)
        {
            ...
            {
                Title = "Confirmacion",
                Text = $"¿Estas seguro de que quieres borrar el empleado {employee.Name}?",
            ...
            var responseHttp = await Repository.DeleteAsync<Employment>($"api/Employees/{employee.Id}");
            if (responseHttp.Error)
            {
                if (responseHttp.HttpResponseMessage.StatusCode == HttpStatusCode.NotFound)
                {
            ...
            await toast.FireAsync(icon: SweetAlertIcon.Success, title: "Empleado borrado exitosamente");
        }
    }
}